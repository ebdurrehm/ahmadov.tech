...
The key is a random string of bits that is used to encrypt and decrypt the data. The key is shared between the sender and receiver. The key is used to encrypt the plain text block to produce a cipher text block and to decrypt the cipher text block to produce the plain text block. It must be kept secret and should be shared between the sender and receiver in a secure way.
...
Let's implement a real-world example of AES encryption in CBC mode using Node.js. We will create a simple chat application that encrypts and decrypts messages using AES encryption in CBC mode. We will use the built-in net module in Node.js to create a TCP server and client that communicate with each other over a secure connection.
...
const ALGORITHM = 'aes-256-cbc';
const SECRET_KEY = Buffer.from(process.env.SECRET_KEY, 'hex'); // 32-byte key
const IV_LENGTH = 16; 
const IV = crypto.randomBytes(IV_LENGTH);

// Encrypts inputFile using AES-256-CBC encryption
function encryptFile(inputFile, outputFile, iv) {
    ...
    console.log("✅ File encrypted successfully! Share this IV separately:", iv.toString('hex'));
}

// Decrypts encryptedFile using AES-256-CBC decryption
function decryptFile(encryptedFile, outputFile, iv) {
...
//  Example usage
const iv = crypto.randomBytes(IV_LENGTH);
encryptFile('input.txt', 'encrypted.dat', IV);
decryptFile('encrypted.dat', 'decrypted.txt', IV);
```

What we did we created simple file encryption algorithm wich encrypt file content and write it to another file. We used the same IV and key for encryption and decryption. We can use same method for storing user bank card details or any sensitive data to database.

### Conclusion

In this article we talked about AES algorithm, CBC mode, and how to encrypt and decrypt data using AES encryption in CBC mode with Node.js. We covered the basics of encryption, symmetric encryption, AES encryption, CBC mode. We also implemented a real-world example of AES encryption in CBC mode using Node.js. I hope you found this article helpful and learned something new about encryption and Node.js. If you have any questions or feedback, feel free to leave a comment below. Thank you for reading!
